...
O slide 29 mostra os QR codes e links publicados do GitHub e do EventCatalog desde o início. Um clique revela “Obrigado!” abaixo deles, sem deslocar os cartões; ao voltar o clique, só o agradecimento desaparece. Os destinos continuam definidos em `lib/published-links.json`, com QR codes de 184px e espaçamento de 96px entre os cartões.
...
O slide 21 separa fontes, geração do conteúdo e construção do site, mantendo três cliques. No segundo, um recorte real de `index.mdx` mostra `orders-create-order` enviando `OrderCreated` em `sends`; o consumidor referencia a mesma mensagem em `receives`. O gerador traduz os YAMLs para recursos e relações em Markdown/MDX; o build do EventCatalog transforma esses arquivos em páginas e mapas. A documentação também pode ser mantida manualmente: geradores são uma opção de automação, não uma etapa obrigatória do produto. O ícone Python representa a implementação relatada no trabalho; o gerador deste repositório usa TypeScript e `@eventcatalog/sdk`. O slide 22 continua com a publicação de `dist/` no S3 e distribuição pelo CloudFront, sem repetir a geração.
